...
const QuizCard: React.FC<QuizCardProps> = ({ quiz }) => {
    ...
    const handleAnswerClick = (index: number): void => {
        ...
        if (selectedAnswer.correct && !isQuestionAnswered) {
            //risposta corretta && non era stata ancora selezionata la risposta
            ...
            //risposta corretta && ma era già stata selezionata la risposta
            if (!isPrevAnswerCorrect) {
                //se la precedente risposta non era corretta allora metti +1 al counter delle corrette, altrimenti non fare niente
                dispatch(rightAnswer());
            }
        } else if (!selectedAnswer.correct && !isQuestionAnswered) {
            //risposta errata && non era stata ancora selezionata la risposta
            dispatch(newAnswer());
        } else if (!selectedAnswer.correct && isQuestionAnswered) {
            //risposta errata && era già stata selezionata la risposta
            if (isPrevAnswerCorrect) {
