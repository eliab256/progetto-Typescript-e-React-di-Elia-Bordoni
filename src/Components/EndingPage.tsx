...
const EndingPage: React.FC = () => {
    ...
    return (
        <div className="endingPageContainer widthController">
            <div className="testResult">{`${CorrectAnswerNumber} / ${TotalAnswerNumber}`}</div>
            <div className="testResultPercent">{`You answered correctly to ${PercentResult()}% of the questions.`}</div>
            <div className="resultComment">{CongratsMessage(PercentResult())}</div>
